...
- **R9 — 2026-05-27:** before MAE=N/A (8-feature baseline not recorded), after MAE=0.1745 raw / 0.0405 rounded (gate ≤0.50 ✅), RMSE=0.2467, exact acc=96.1%, within±1=99.8% (50k rows). Notes: recentAvgDifficulty (index 8) added to TargetChangeFeatures; grinding-suppressor rule fires when recentAvgDifficulty≥4.0f AND rate30d≥0.80f → delta=−1 in both TFLite (TARGET_CHANGE_FEATURE_COUNT 8→9) and MathHabitPredictor (Rule 0, highest priority); default 3.0f when <3 rated completions; MIN_DIFFICULTY_RATINGS=3, MAX_DIFFICULTY_COMPLETIONS=14; 3 JUnit tests added to MathHabitPredictorTest (grinding suppressor fires, increase fires when difficulty low, neutral=0).
- **R10 — 2026-05-27:** before MAE=N/A (12-feature baseline not recorded), after MAE=0.0495, RMSE=0.0626 (gate ≤0.12 ✅, beats naive MAE 0.0614), 27 epochs, 50k rows. Notes: 5 new features added to WeeklyForecastFeatures (clusterProportionEffortless/Consistent/Struggling/Dormant, avgAbandonmentRisk); WeeklyForecastUseCase injects BehavioralClusterUseCase + AbandonmentRiskUseCase, loops habits to compute normalized cluster proportions and mean abandonment probability; MathHabitPredictor fallback updated to clip(base − 0.10 × avgAbandonmentRisk, 0, 1); StatisticsViewModel declaration order fixed (behavioralClusterUseCase before weeklyForecastUseCase), skips fetched via habitSkipDao.getAllRecent(30d); WeeklyForecastUseCaseTest added (3 tests, all green).