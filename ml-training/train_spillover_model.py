"""
train_spillover_model.py — Train + export Phase 8.5 (SpilloverRegressor).

PLAN-ML-EXTENSION.md §8.5.1.

This is a REGRESSION pipeline producing lift_delta ∈ [-0.5, +0.5].
Differences vs the [0,1]-bounded forecasters (Phase 8.3):

    * Output range      : [-0.5, +0.5]  (not [0,1])
    * Output layer      : Dense(1, tanh) + Lambda(x * 0.5)
                          tanh bounds to [-1,1]; the Lambda scales to target range.
                          Both ops are TFLite-compatible (Mul by constant).
    * Loss              : MAE (mean absolute error)
    * Acceptance gate   : MAE <= 0.08 on the test set.
    * Retry policy      : 50k rows first; retry at 80k if gate not met.

Pipeline:

    1. Load (or auto-generate) ml-training/data/spillover_dataset.csv.
    ...
    9. Write models/spillover_regressor.tflite.

Outputs:
    ml-training/models/spillover_regressor.tflite
    ml-training/models/spillover_scaler.json

Usage:
    python train_spillover_model.py
"""
...
import generate_spillover_data as gen  # noqa: E402
...
# ---------------------------------------------------------------------------
# Acceptance thresholds (PLAN-ML-EXTENSION.md §8.5, adapted for regression)
# ---------------------------------------------------------------------------
MAX_MAE = 0.08
INITIAL_ROWS = 50_000
RETRY_ROWS = 80_000

LABEL_COL = "lift_delta"
...
def _build_model(n_features: int) -> tf.keras.Model:
    """Small MLP with tanh * 0.5 output to enforce the [-0.5, +0.5] output range.

    Architecture mirrors the other regression model (Phase 8.3) for thesis
    consistency.  The Lambda layer multiplies the tanh output by 0.5; both
    tanh and Mul-by-scalar are fully TFLite-compatible ops.
    """
    model = tf.keras.Sequential(
        [
            ...
            # tanh → [-1, 1]; then scale to [-0.5, +0.5].
            tf.keras.layers.Dense(1, activation="tanh"),
            tf.keras.layers.Lambda(lambda x: x * 0.5, name="lift_delta"),
        ],
...
def _train_once(rows: int) -> tuple[tf.keras.Model, StandardScaler, float, float]:
    ...
    # Naive baseline: always predict 0 (no spillover) — MAE = mean(|lift_delta|).
    naive_mae = float(mean_absolute_error(y_test, np.zeros_like(y_test)))

    print(f"Test MAE   : {test_mae:.4f}")
    print(f"Test RMSE  : {test_rmse:.4f}")
    print(f"Naive MAE  : {naive_mae:.4f}   (predicting 0 lift for all pairs)")
...
def _save_scaler(scaler: StandardScaler) -> Path:
    ...
    Field order must exactly mirror SpilloverFeatures.kt → toFloatArray()
    ...
    out = _models_dir() / "spillover_scaler.json"
...
def _export_tflite(model: tf.keras.Model) -> Path:
    ...
    out = _models_dir() / "spillover_regressor.tflite"
...
def main() -> None:
    ...
    if test_mae > MAX_MAE:
        raise SystemExit(
            ...
            "Review label-generation rules in generate_spillover_data.py "
