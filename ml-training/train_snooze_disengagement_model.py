"""
train_snooze_disengagement_model.py — Train + export Phase 9.2 (SnoozeDisengagementClassifier).

PLAN-ML-EXTENSION.md §9.2.2. Pipeline:

    1. Load (or auto-generate) ml-training/data/snooze_disengagement_dataset.csv.
    2. 80/20 train/test split (stratified on label) via sklearn.
    3. Fit StandardScaler on training features; persist mean & scale to
       models/snooze_disengagement_scaler.json so Android can apply IDENTICAL
    ...
    4. Build the Keras MLP:
       ...
       Same architecture as all binary classifiers in this project
       (PLAN-ML-EXTENSION.md Global Standards).
    ...
    7. Evaluate on the held-out test set. Acceptance threshold:
           F1 >= 0.75   (mirrors Phase 8.1 mandate in PLAN-ML-EXTENSION.md)
    ...
    9. Write models/snooze_disengagement_classifier.tflite.

Outputs:
    ml-training/models/snooze_disengagement_classifier.tflite
    ml-training/models/snooze_disengagement_scaler.json

Usage:
    python train_snooze_disengagement_model.py
"""
...
import generate_snooze_disengagement_data as gen  # noqa: E402
...
# ---------------------------------------------------------------------------
# Acceptance thresholds (mirrors PLAN-ML-EXTENSION.md §8.1.2 convention)
# ---------------------------------------------------------------------------
MIN_F1 = 0.75
INITIAL_ROWS = 50_000
RETRY_ROWS = 80_000
...
def _build_model(n_features: int) -> tf.keras.Model:
    ...
    Architecture is identical to HabitAbandonmentClassifier (Phase 8.1) and
    HabitSuccessClassifier (Phase 6.5) for consistency across the thesis.
    """
    model = tf.keras.Sequential(
        [
            ...
            tf.keras.layers.Dense(
                1, activation="sigmoid", name="disengagement_probability"
            ),
        ],
...
def _train_once(rows: int) -> tuple[tf.keras.Model, StandardScaler, float, float, float]:
    ...
    # class_weight balances the ~60/40 engaged/disengaging split so the model
    # does not learn to blindly predict "engaged" for all samples.
    # Weight = neg_rate / pos_rate — each disengagement error is penalized more,
    # boosting recall on the at-risk minority class. Same rationale as 8.1.
    ...
    # Macro-averaged F1 gives equal weight to both classes (engaged and
    # disengaging), matching the acceptance mandate convention used in 8.1.
    ...
    print()
    print(classification_report(y_test, y_pred_label, target_names=["engaged", "disengaging"]))
...
def _save_scaler(scaler: StandardScaler) -> Path:
    ...
    ``(x - mean) / scale`` per feature before running the interpreter.
    Field order must exactly mirror SnoozeDisengagementFeatures.kt → toFloatArray().
    """
    payload = {
    ...
    out = _models_dir() / "snooze_disengagement_scaler.json"
...
def _export_tflite(model: tf.keras.Model) -> Path:
    ...
    out = _models_dir() / "snooze_disengagement_classifier.tflite"
...
def main() -> None:
    ...
    if f1 < MIN_F1:
        print(
            ...
            f"Retrying with {RETRY_ROWS:,} rows per PLAN-ML-EXTENSION.md §9.2.2."
        )
    ...
    print()
    print("=" * 60)
    print("SUCCESS — Phase 9.2 (SnoozeDisengagementClassifier) ready.")
    print(f"  test accuracy : {accuracy:.4f}")
    print(f"  ROC-AUC       : {auc:.4f}")
    print(f"  Macro F1      : {f1:.4f}")
    ...
    print("      then implement TfliteHabitPredictor.predictSnoozeDisengagement.")
    print("=" * 60)
