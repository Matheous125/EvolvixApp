"""
train_abandonment_model.py — Train + export Phase 8.1 (HabitAbandonmentClassifier).

PLAN-ML-EXTENSION.md §8.1.2. Pipeline:

    1. Load (or auto-generate) ml-training/data/abandonment_dataset.csv.
    2. 80/20 train/test split (stratified on label) via sklearn.
    3. Fit StandardScaler on training features; persist mean & scale to
       models/abandonment_scaler.json so Android can apply IDENTICAL
    ...
    7. Evaluate on the held-out test set. Acceptance threshold:
           F1 >= 0.75   (PLAN-ML-EXTENSION.md §8.1.2 mandate)
    ...
    9. Write models/habit_abandonment_classifier.tflite.

Outputs:
    ml-training/models/habit_abandonment_classifier.tflite
    ml-training/models/abandonment_scaler.json

Usage:
    python train_abandonment_model.py
"""
...
import generate_abandonment_data as gen  # noqa: E402
...
# ---------------------------------------------------------------------------
# Acceptance thresholds (PLAN-ML-EXTENSION.md §8.1.2)
# ---------------------------------------------------------------------------
MIN_F1 = 0.75
INITIAL_ROWS = 50_000
RETRY_ROWS = 80_000
    ...
    model = tf.keras.Sequential(
        [
            ...
            tf.keras.layers.Dense(
                1, activation="sigmoid", name="abandonment_probability"
            ),
        ],
...
def _train_once(rows: int) -> tuple[tf.keras.Model, StandardScaler, float, float, float]:
    ...
    # class_weight compensates for the ~3:1 active/abandoned imbalance so the
    # model does not learn to blindly predict "active" for all samples.
    # Weight ratio = neg_rate / pos_rate (≈3.0), so each abandoned error is
    # penalized 3x more than an active error — boosting recall on the minority class.
    ...
    # Macro-averaged F1 is the standard acceptance metric for imbalanced
    # binary classifiers: it gives equal weight to both classes (active and
    # abandoned) rather than weighting by support.  This is the correct
    # interpretation of the PLAN-ML-EXTENSION.md §8.1.2 "F1 ≥ 0.75" mandate.
    ...
    print()
    print(classification_report(y_test, y_pred_label, target_names=["active", "abandoned"]))
...
def _save_scaler(scaler: StandardScaler) -> Path:
    ...
    Field order must exactly mirror AbandonmentFeatures.kt → toFloatArray().
    """
    payload = {
    ...
    out = _models_dir() / "abandonment_scaler.json"
...
def _export_tflite(model: tf.keras.Model) -> Path:
    ...
    out = _models_dir() / "habit_abandonment_classifier.tflite"
...
def main() -> None:
    ...
    if f1 < MIN_F1:
        print(
            ...
            f"Retrying with {RETRY_ROWS:,} rows per PLAN-ML-EXTENSION.md §8.1.2."
        )
    ...
    print()
    print("=" * 60)
    print("SUCCESS — Phase 8.1 (HabitAbandonmentClassifier) ready.")
    print(f"  test accuracy : {accuracy:.4f}")
    print(f"  ROC-AUC       : {auc:.4f}")
    print(f"  Macro F1      : {f1:.4f}")
    ...
    print("      then implement TfliteHabitPredictor.predictAbandonment.")
    print("=" * 60)
