...
│       ├── ShouldResetHabitUseCase.kt      # Pure use-case (Phase 9.6.1): operator fun invoke(habit, now: LocalDateTime): Boolean; encapsulates per-frequency reset predicates (Daily/Weekly/Monthly) extracted from HabitViewModel.checkAndResetProgress(); called by RecordHabitActionWorker before incrementing to prevent reset-race condition on Done-from-notification; also used by HabitViewModel
...
│   ├── HabitActionReceiver.kt     # BroadcastReceiver for Done/Snooze taps on reminder notifications (Command pattern); Phase 9.6.2: Done enqueues RecordHabitActionWorker via WorkManager (replaces direct Room write); ACTION_SKIP removed — Skip button routes directly to SkipReasonPickerActivity via PendingIntent.getActivity()
