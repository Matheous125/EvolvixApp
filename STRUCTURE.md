...
│   │   ├── AppDatabase.kt       # Room Database setup (singleton via companion object); version = 19 after Phase 9.4 schema change
...
│   │   ├── HabitPredictor.kt      # Interface defining all predictive + passive-analytics contracts (success probability, optimal time, routine precision, behavioral clustering, etc.); Phase 9.4 adds predictPerceivedDifficulty(DifficultyFeatures): Float
...
│   │   ├── TfliteHabitPredictor.kt # TFLite implementation of HabitPredictor; K-Means clustering via habit_clusters.json (nearest-centroid, no Interpreter); Phase 9.4 adds difficultyInterpreter loading perceived_difficulty_regressor.tflite + perceived_difficulty_scaler.json; falls back to MathHabitPredictor
...
│   │   ├── StatisticsScreen.kt    # Analytics: weekly overview, life balance, per-habit stats, AI insight cards; Phase 9.4 adds PerceivedDifficultyCard (ElevatedCard) collecting difficultyEstimates StateFlow; DifficultyRow shows predicted ★, Rating chip, and optional user avg
...
│       ├── StatisticsViewModel.kt          # Combines habits+completions Flows; runs all analytics use cases; exposes overview, lifeBalance, perHabitStats, AI cards as StateFlows; Phase 9.4 adds difficultyEstimates: StateFlow<Map<Int, PerceivedDifficultyEstimate>>
...
└── evaluate_models.py              # Thesis-grade evaluation report: loads .tflite artifacts + habit_clusters.json, reproduces test splits, computes metrics + silhouette + PCA scatter, saves plots to data/plots/; Phase 9.4 adds PerceivedDifficultyRegressor evaluation block
```